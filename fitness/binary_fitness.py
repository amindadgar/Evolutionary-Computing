"fitness functions for binary genes in evolutionary algorithm"
...
def fourpeaks(string_gene, T) -> int:
    """
    ...
    fitness = max(tail(string_gene, '0'), head(string_gene, '1')) + R(string_gene, T, 'fourpeak')

    return fitness

def sixpeaks(string_gene, T) -> int:
    """
    ...
    fitness = max(tail(string_gene, '0'), head(string_gene, '1')) + R(string_gene, T, 'sixpeak')

    return fitness

def trap(string_gene):
    """
    ...
    fitness : int
        the fitness of the gene
    """
    fitness = 3 * len(string_gene) * peak(string_gene) - onemax(string_gene)

    return fitness


def R(string_gene, T, fitness_function):
    """
    ...
    if fitness_function == 'sixpeak':
        ...
        condition1 = (tail(string_gene, '0') > T) or head(string_gene, '1') > T
        condition2 = (tail(string_gene, '1') > T) or head(string_gene, '0') > T

        condition = condition1 or condition2

    elif fitness_function == 'fourpeak':
        condition = (tail(string_gene, '0') > T) or head(string_gene, '1') > T
    else:
...
def tail(string_gene, value):
    ...
    count = head(reversed_string_gene, value)

    return count


def head(string_gene, value):
    """
